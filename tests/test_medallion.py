...
EXPECTED_KPIS = {
    ...
    "closed_requests": 73,
    "open_backlog": 16,
    "sla_eligible_requests": 73,
    "sla_met_requests": 31,
    "sla_compliance_rate": 0.4247,
    "average_resolution_minutes": 1653.75,
    "median_resolution_minutes": 1320.0,
    "escalated_requests": 10,
    "escalation_rate": 0.1124,
    "reopened_requests": 19,
    "reopen_rate": 0.2135,
}
...
def test_bronze_preserves_source_rows_and_adds_metadata(tables) -> None:
    assert len(tables.bronze) == 100
    assert tables.bronze["source_row"].tolist() == list(range(2, 102))
    assert tables.bronze["source_file"].unique().tolist() == ["service_requests.csv"]
    assert tables.bronze["ingestion_batch_id"].unique().tolist() == ["0975185eb13ee5f5"]


def test_silver_reconciles_valid_and_rejected_rows(tables) -> None:
    assert len(tables.silver_valid) == 89
...
def test_write_medallion_persists_all_layers(tables, tmp_path: Path) -> None:
    ...
    assert len(pd.read_parquet(output_dir / "bronze/service_requests.parquet")) == 100
    assert len(pd.read_parquet(output_dir / "silver/service_requests_valid.parquet")) == 89
    assert len(pd.read_parquet(output_dir / "gold/fact_service_requests.parquet")) == 89
